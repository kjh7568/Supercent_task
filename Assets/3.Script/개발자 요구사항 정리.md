...
## TransportWorkerSystem 운반 인부 시스템
...
## 리팩토링 - PickupZone 내 아이템 생성 시 즉시 이동 버그
...
## 리팩토링 - DepositZone/PickupZone 이동 버그 수정
...
## 구매 완료 시 메테리얼 변경 (2026-04-06)

**요청 내용**
구매 완료 손님의 메테리얼을 지정된 것으로 변경. Renderer가 자식 오브젝트에 있음.

**구현 방법**
- `CustomerConfig`에 `purchasedMaterial` 필드 추가
- `Customer.CompletePurchase()`에서 `GetComponentInChildren<Renderer>()`로 자식 Renderer 탐색 후 `material` 교체
- `purchasedMaterial`이 null이면 변경 없이 통과

---

## 손님 애니메이터 컨트롤러 (2026-04-06)

**요청 내용**
Speed 파라미터로 Idle ↔ Move 전환하는 CustomerAnimatorController 생성.

**구현 방법**
- `CustomerAnimatorController.cs` 신규 생성 (Customer/ 폴더)
- 조이스틱 없이 `MoveTowards`로 이동하는 구조에 맞춰 매 프레임 이전 위치 대비 이동 거리로 실제 속도 계산
- `_animator.SetFloat("Speed", speed)` 전달

---

## 손님 요구량 랜덤화 (2026-04-06)

**요청 내용**
손님 요구량이 3 고정이 아닌 랜덤 범위로 결정되게 수정.

**구현 방법**
- `CustomerConfig.demandAmount` → `minDemand` / `maxDemand` 필드로 교체
- `Customer.Initialize()`에서 `Random.Range(minDemand, maxDemand + 1)`로 결정, `DemandAmount` 프로퍼티로 노출
- `CashRegister.cs`, `CustomerHUDElement.cs`, `UIManager.cs` 참조를 `customer.DemandAmount`로 교체

---

## 채굴 애니메이션 속도 연동 + 곡괭이 표시 (2026-04-06)

**요청 내용**
채광 중에만 곡괭이 표시, 채광 단계별 애니메이션 속도 연동 (기본 1s → 0.6s → 0.3s).

**구현 방법**
- `PlayerAnimationController`에 `pickaxe` GameObject 필드 추가, `isMining` 여부에 따라 `SetActive` 토글
- `baseMiningInterval = 1f` 기준으로 `_animator.speed = baseMiningInterval / miningController.MiningInterval` 공식 적용
- 채굴 비활성 시 `_animator.speed = 1f`로 복원

---

## 리팩토링 - 이벤트 구독 라이프사이클 개선 (2026-04-06)
